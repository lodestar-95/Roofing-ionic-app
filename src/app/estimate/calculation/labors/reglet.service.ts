...
export class RegletService {
    ...
    async calculateLabor(building, materialCalculations) {
        ...
        const labor_category_ridglet = await this.general.getConstDecimalValue('labor_category_ridglet');
        const labor_price_ridglet = await this.general.getConstDecimalValue('labor_price_ridglet');

        const laborPrices = (await this.catalogs.getLaborPrices()).data;
        const laborPrice = laborPrices.find(price => price.id == labor_price_ridglet);

        for (const shingle of shingles) {
            const quantity = materialCalculations.filter(material => material?.id_material_category == category_ridglet
            ...
            laborCalculation.category = labor_category_ridglet;
