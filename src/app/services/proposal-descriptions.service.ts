...
export class ProposalDescriptionsService {
  ...
  async getTagValue(tag: string) {
    switch (tag) {
      ...
      case '[QtyMetalVents]':
        return await this.quantityMetalVents();
      case '[QtyMetalVents4]':
  ...
  async addOrReplaceMetalVents() {
    ...
    if (this.building?.psb_measure?.vent_metal_artict_replace_pc == 0 &&
        ...
        this.building?.psb_measure?.vent_j_vent_4_pc == 0 &&
        this.building?.psb_measure?.vent_j_vent_6_pc == 0) {
  ...
  async quantityMetalVents() {
    const categoryMetalAttic = await this.general.getConstDecimalValue('category_metal_artict_vents');
    //const metalArtictVent = this.general.getFirstMaterial(this.materials, categoryMetalAttic);
